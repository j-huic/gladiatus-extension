...
When enabled on a Gladiatus Guild Market page, the extension reads the item staged in the game's sell form so it can match Mini-Pumpkin and calculate its stack price. A matching price is filled automatically, the native duration is set to 24 hours, and the displayed fee is recalculated. The extension does not submit the form or list an item.
